...
gracefully. If the ``company_launch`` auxiliary slot is not configured,
...
def run_ceo_turn(
    *,
    ...
    the ``company_launch`` slot, degrades gracefully when unconfigured).
...
def aux_configured() -> bool:
    """True only when the ``company_launch`` slot is explicitly set.
