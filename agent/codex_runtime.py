        ...
        try:
            with active_client.responses.stream(**api_kwargs) as stream:
                try:
                    ...
                    # ChatGPT backend sends valid stream events but the SDK's
                    # internal state machine raises TypeError when it tries to
                    # iterate over response.output=None during parsing
                    # (openai/lib/_parsing/_responses.py:61).
                    # Fall through to synthesize response from collected events.
                    logger.debug(
                        "Codex Responses stream iteration crashed with TypeError "
                        "(SDK parse error — no terminal event). "
                        "collected_items=%d, text_parts=%d. %s",
                        len(collected_output_items),
                        len(agent._codex_streamed_text_parts),
                        agent._client_log_context(),
                    )
                except RuntimeError:
                    # The stream may also raise RuntimeError when the SDK's
                    # state machine detects missing terminal events.
                    # Same recovery: synthesize from collected events.
                    logger.debug(
                        "Codex Responses stream iteration raised RuntimeError. "
                        "collected_items=%d, text_parts=%d. %s",
                        len(collected_output_items),
                        len(agent._codex_streamed_text_parts),
                        agent._client_log_context(),
                    )
                # Try get_final_response() if we got a clean stream,
                # otherwise synthesize from collected events.
                try:
                    ...
                    pass
                # Synthesize response from collected stream events
                if collected_output_items:
                    ...
                    logger.debug(
                        "Codex stream: synthesized response from %d collected output items",
                        len(collected_output_items),
                    )
                    ...
                    # Nothing collected — let the outer retry loop handle it
                    raise RuntimeError(
                        "Codex stream produced no output items or text deltas"
                    )
                return final_response
